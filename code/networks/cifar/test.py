...
class YoloV3_test(CifarModel):
    ...
    def DarknetTiny(self,x,name=None):
    ...
    def YoloConvTiny(self, x, filters):
    ...
    def YoloV3Tiny(self, input_img):
        # x = inputs = Input([size, size, channels], name='input')
        # x = inputs = input_img
        inputs = Input(shape=input_img.shape[1:], name='input')
        x = inputs
        x_8, x = self.DarknetTiny(x,name='yolo_darknet')

        x = self.YoloConvTiny(x, 256)
    ...
    def network(self, img_input):
        ...
        return self.YoloV3Tiny(img_input)
