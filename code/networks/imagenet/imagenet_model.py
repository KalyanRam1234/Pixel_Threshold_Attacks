...
class ImagenetModel(Model):
    ...
    def dataset(self):
        ...
        def parser(filename, image, label, istrain): 
            if self.raw_defence: 
        ...
        def create_dataset(is_training):                
            ...
            raw_dataset = raw_dataset.map(lambda value: parse_record(value, is_training), num_parallel_calls=tf.data.experimental.AUTOTUNE)
            raw_dataset = raw_dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
